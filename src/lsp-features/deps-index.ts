...
export class DepsIndex {
  ...
  private async calculateImportsAndSaveCache(document: TextDocument, tree: Parser.Tree) {
    ...
    if (tolkSdk) {
      ...
      await this.resolveImport(document.uri, '@stdlib/common.tolk');
    ...
    for (let node of tree.rootNode.children.filter(n => n.type === 'import_directive')) {
      ...
      let importUri = await this.resolveImport(document.uri, pathNodeText, tolkSdk ?? undefined)
  ...
  async resolveImport(documentUri: string, pathNodeText: string, compilerSdk?: TolkCompilerSDK): Promise<string | null> {
    let tolkSdk = compilerSdk ?? await this._tolkSdkMapping.getOrRequestTolkSdkForFile(documentUri)

    let importUri = resolveImport(documentUri, pathNodeText, tolkSdk)

    let entry: TextDocument | undefined;
    if(importUri) {
      entry = importUri ? await this._documents.retrieve(importUri) : undefined;
    }
  ...
  async getImports(document: TextDocument): Promise<DocumentImports['importedUris']> {
    ...
    const gotImports = this._cachedImports.get(document.uri);
    return gotImports!.importedUris;
  }
